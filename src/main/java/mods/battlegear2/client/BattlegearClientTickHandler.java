...
public final class BattlegearClientTickHandler {
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        if (event.player == mc.thePlayer) {
            if (event.phase == TickEvent.Phase.START) {
                if (ticksBeforeUse > 0) ticksBeforeUse--;
    ...
    private boolean onPlayerPlaceBlock(PlayerControllerMP controller, EntityPlayer player, ItemStack offhand, int i,
        ...
        if (player.getCurrentEquippedItem() != null) {
            if ((block instanceof BlockLog && player.getCurrentEquippedItem()
                .getItem() instanceof ItemAxe) || (block instanceof BlockGrass
            ...
            if (controller.isInCreativeMode()) {
                ...
                if (flag1) {
                    BattlemodeHookContainerClass
                        .sendOffSwingEventNoCheck(player, offhand, player.getCurrentEquippedItem());
                }
                return flag1;
            } else {
    ...
    @SideOnly(Side.CLIENT)
    public static void tryBreakBlockOffhand(MovingObjectPosition objectMouseOver, ItemStack offhandItem,
        ItemStack mainHandItem, TickEvent.PlayerTickEvent event) {
        ...
        if (mcInstance.thePlayer.capabilities.isCreativeMode) {
            if (ClientTickHandler.delay <= 0) {
                ...
                if (!(BattlegearUtils.usagePriorAttack(offhandItem))
                    ...
                    BattlemodeHookContainerClass.sendOffSwingEventNoCheck(event.player, mainHandItem, offhandItem); // force
                                                                                                                    // offhand
                                                                                                                    // swing
                                                                                                                    // anyway
                                                                                                                    // because
        ...
        if (mcInstance.theWorld.getBlock(i, j, k)
            ...
            if (mcInstance.playerController.blockHitDelay > 0) {
                ...
                if (mcInstance.playerController.currentBlockX != i || mcInstance.playerController.currentBlockY != j
                ...
                if (offhandItem != null) {
                    ...
                    mcInstance.playerController.currentItemHittingBlock = BattlegearUtils
                ...
                mcInstance.theWorld.destroyBlockInWorldPartially(
                    mcInstance.thePlayer.getEntityId(),
                    mcInstance.playerController.currentBlockX,
                    mcInstance.playerController.currentBlockY,
                    mcInstance.playerController.currentblockZ,
                    (int) (mcInstance.playerController.curBlockDamageMP * 10.0F) - 1);
            }
